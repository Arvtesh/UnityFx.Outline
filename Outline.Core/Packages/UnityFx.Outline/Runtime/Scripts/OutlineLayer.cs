...
namespace UnityFx.Outline
{
	...
	public sealed class OutlineLayer : ICollection<GameObject>, IReadOnlyCollection<GameObject>, IOutlineSettings
	{
		...
		{
			private readonly Dictionary<GameObject, OutlineRendererCollection> _objects;

			public EditorList(OutlineLayer layer)
			{
				_objects = layer._outlineObjects;
			}
			...
			public object this[int index]
			{
				get
				{
					...
					foreach (var go in _objects.Keys)
					{
						if (myIndex == index)
						{
							return go;
			...
			public bool Contains(object value)
			{
				...
				{
					return _objects.ContainsKey(go);
				}
		...
		private EditorList _editorObjects;
		...
		{
			if (_editorObjects is null)
			{
				_editorObjects = new EditorList(this);
			}
